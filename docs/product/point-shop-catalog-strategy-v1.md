...
WHICH-117의 1차 목표는 포인트 사용처를 실제로 열 수 있는 안전한 상점 기반을 만드는 것입니다. 상품 수를 먼저 늘리기보다 Catalog → 구매 → Inventory → 장착 → 환불이 하나의 원장 흐름으로 일관되게 동작하는 것을 우선합니다.
...
- 환불은 기존 원장 행을 수정하지 않고 별도의 `REFUND` 원장 행과 Inventory 회수 기록으로 남깁니다.
