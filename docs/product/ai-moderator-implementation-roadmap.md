...
- Status: v1 backlog — assisted moderation only
- Last updated: 2026-08-26
- Notion plan: [WHICH AI Moderator 구현 계획 — 1인 운영 보조형 Moderation](https://app.notion.com/p/3c828b27a559818d9d2bd597065b6086)
...
이 문서는 `WHICH_AI_MODERATOR_ARCHITECTURE_v1.md` 검토 결과를 WHICH의 현재 코드와 운영
환경에 맞게 재구성한 구현 경계와 Backlog를 저장소에 보관한다. 목표는 운영자를 없애는 것이
아니라, 한 명의 운영자가 검토해야 할 양과 반복 작업을 줄이면서 되돌릴 수 없는 결정을 사람에게
남기는 것이다.

## 결론

WHICH에 AI Moderator를 적용할 수 있다. 다만 첫 출시 형태는 자율 운영자가 아니라 다음 역할을
수행하는 운영 보조 시스템이어야 한다.
...
| WHICH-105 | P1       | 사용자 Issue Moderation 적용                |
| WHICH-106 | P2       | Policy LLM·Fine-tuning 도입 평가            |
| WHICH-107 | P2       | Vote Fraud 보조 신호와 검증                 |
| WHICH-108 | P2       | 이미지 OCR·Safety·Rights Gate               |
| WHICH-109 | P2       | 프로필 Moderation                           |
| WHICH-110 | P3       | 계정 제한·Cooldown·Risk Decay               |
...
WHICH-106~110은 Shadow Mode와 실제 운영 데이터가 필요성을 입증한 항목만 승격한다.
