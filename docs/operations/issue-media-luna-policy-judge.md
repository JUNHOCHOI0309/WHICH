...
상태: 2026-08-30 최초 OFF 도입 기록. 2026-09-02 현재 Member 자동 공개 운영 활성.
[현재 공개·감사 기준](./which-105-111-member-auto-publication-audit.md).

이 문서의 OFF·PILOT 설명은 최초 도입 당시의 안전 기본값과 단계별 검증 기록이다. 현재 운영은
로컬 검사와 OpenAI Moderation을 통과한 제출에 Luna를 적용하고, Luna `ALLOW`와 관련성·균형·
개인정보·권리 LOW를 모두 만족한 현재 revision만 자동 공개한다. 실패·불완전·`ABSTAIN`은 비공개
상태로 유지한다.
