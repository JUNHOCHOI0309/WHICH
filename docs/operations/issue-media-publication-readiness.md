...
2026-08-30 후속: 현재 제출·계정·권한·동의를 읽는 [내부 근거 해석기](./issue-media-publication-evidence.md)를
결정 엔진에 연결했다. `decisionAssessment`가 필수 9종의 실제 상태와 결정 결과를 반환한다.
검증된 저위험 PASS 신호와 시각 검사 공급은 아직 없으며, 해석기 연결만으로 공개를 허가하지 않는다.
