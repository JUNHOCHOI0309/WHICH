...
- Cloud Run Direct VPC/NAT 초기 연결 지연을 고려하여 DB 연결 제한은 20초이며, 수집 시작 전 읽기 전용 준비 확인을 최대 4회(간격 5초) 시도합니다. 수집 작업 자체를 통째로 재시도하지 않으며, 준비 실패 시 `DATABASE_NOT_READY`로 종료합니다.
