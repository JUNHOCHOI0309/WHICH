...
네이버 카페·블로그·홈피드DA는 특정 Issue 딥링크를 사용한다. 네이버 클립처럼 외부 URL을
직접 연결하기 어려운 Surface는 `클립 → 네이버 블로그 → WHICH Issue` 경로를 사용한다.
모든 네이버 링크는 채널·캠페인·Issue를 구분하는 UTM을 포함하되 개인정보를 넣지 않는다.
네이버 로그인은 첫 투표 전 필수 단계가 아니며, Result 이후 선택 가능한 Member 연결 수단이다.
