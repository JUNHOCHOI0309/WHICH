...
```text
네이버 CHOiCE / 카페 / 클립→블로그 / 홈피드DA / X / YouTube
→ 특정 Issue URL
→ 비회원 즉시 투표
→ 결과 확인
→ 결과 카드·링크 공유
→ 네이버 콘텐츠·커뮤니티와 외부 채널 재노출
→ 신규 사용자
```

**[확정]** 한국 초기 Acquisition은 Instagram 직접 유입보다 네이버 생태계의 질문 검증,
커뮤니티, 검색·콘텐츠, 유료 직접 유입을 우선한다. 네이버 로그인은 이 유입 신호와 분리된
Member 인증 수단이며, 로그인하지 않은 Guest의 첫 투표를 막지 않는다.
...
- 네이버 내부 반응과 WHICH Vote Fact를 같은 표본으로 합산하지 않는다.
- 카페·블로그 정책을 위반하는 링크 도배나 기계적 대량 게시를 허용하지 않는다.
...
- 도메인
- 최소 이용 연령
- 네이티브 앱 정식 배포 시점과 Android·iOS 출시 순서
