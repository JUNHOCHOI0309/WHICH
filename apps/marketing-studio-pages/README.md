...
- 이미지 카드는 생성하지 않습니다.
- 선택한 WHICH 질문으로 5초 세로형 HyperFrames 제작 패키지를 비용 없이 생성할 수 있습니다. 스튜디오는 렌더 입력 JSON을 만들고, 실제 MP4 렌더는 로컬 `apps/marketing-hyperframes`에서 수행합니다.
...
기본 템플릿은 1080×1920, 30fps, 5초 무음 키네틱 타이포그래피입니다. 외부 영상·이미지·TTS API를 호출하지 않으며 확인되지 않은 투표 수치도 사용하지 않습니다.
