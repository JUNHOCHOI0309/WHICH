...
async function downloadVideo(v){let r;try{r=await fetch(LOCAL_RENDERER+'/render',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({id:v.id,variables:v.variables})})}catch{throw Error('LOCAL_RENDERER_OFFLINE')}if(!r.ok)throw Error(r.status===409?'LOCAL_RENDERER_BUSY':'LOCAL_RENDERER_FAILED');const blob=await r.blob();const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='which-short-'+v.id.slice(0,8)+'-5s.mp4';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000)}
...
function updateButton(){el('generate').disabled=!state.selected;el('generate').textContent='콘텐츠 생성';el('video').disabled=!state.selected;el('video').textContent='5초 쇼츠 만들기'}
...
el('video').onclick=async()=>{if(!state.selected)return;el('video').disabled=true;el('video').textContent='준비 중…';try{const d=await api('/api/video-packages',{method:'POST',body:JSON.stringify({sourceId:state.selected.id,date:el('date').value,slot:el('slot').value})});state.video=d.package;renderResult();el('video').textContent='영상 렌더 중…';setStatus('이 PC에서 5초 쇼츠를 렌더링하고 있습니다. 약 10~20초 걸립니다.');await downloadVideo(state.video);setStatus('5초 쇼츠를 만들고 MP4 다운로드를 시작했습니다.');showToast('5초 쇼츠 다운로드를 시작했습니다.')}catch(e){if(e.message==='LOCAL_RENDERER_OFFLINE'){setStatus('로컬 렌더러가 꺼져 있습니다. PC의 WHICH HyperFrames Renderer를 실행해 주세요.',true);showToast('로컬 렌더러에 연결하지 못했습니다.')}else if(e.message==='LOCAL_RENDERER_BUSY'){setStatus('다른 쇼츠를 렌더링 중입니다. 완료 후 다시 눌러 주세요.',true);showToast('로컬 렌더러가 사용 중입니다.')}else{setStatus(e.message,true);showToast('쇼츠를 생성하지 못했습니다.')}}finally{updateButton()}};
async function init(){
