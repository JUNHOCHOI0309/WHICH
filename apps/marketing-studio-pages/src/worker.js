...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (request.method === "GET" && url.pathname === "/api/youtube-candidates/export")
        return json({ candidates: await readCandidates(env) }, 200, {
          ...headers,
          "content-disposition": 'attachment; filename="legacy-poll-candidates.json"',
        });
      if (request.method === "POST" && url.pathname === "/api/youtube-candidates/import")
        return json(
          { error: "POLL_CANDIDATES_MOVED_TO_OPS", url: `${WHICH_ORIGIN}/ops?tab=polls` },
          410,
          headers,
        );
...
function securityHeaders() {
  return {
    ...
    "content-security-policy":
      "default-src 'self'; img-src 'self' https: data:; style-src 'unsafe-inline'; script-src 'unsafe-inline'; connect-src 'self' http://127.0.0.1:8783; frame-ancestors 'none'; base-uri 'none'; form-action 'self'",
  };
}
...
function tabs(){document.querySelectorAll('.q-tab').forEach(b=>b.classList.toggle('active',b.dataset.view===state.view))}
...
function renderSources(){
  tabs();const q=el('search').value.trim().toLowerCase();
...
function updateButton(){el('generate').disabled=!state.selected;el('generate').textContent='콘텐츠 생성';const video=el('video');video.textContent='5초 쇼츠 만들기';if(!state.selected){video.removeAttribute('href');video.classList.add('disabled');video.setAttribute('aria-disabled','true');return}const query=new URLSearchParams({sourceId:state.selected.id,date:el('date').value,slot:el('slot').value});video.href=LOCAL_RENDERER+'/render-source?'+query;video.classList.remove('disabled');video.setAttribute('aria-disabled','false')}
function videoCard(){if(!state.video)return '';const v=state.video;return '<div class="video-card"><div class="brand">HYPERFRAMES / 5 SEC</div><div class="output-title">5초 쇼츠</div><p>질문 → A → B → WHICH 선택 유도의 4장면 MP4입니다. 버튼을 누르면 이 PC에서 무료로 렌더링하고 바로 다운로드합니다.</p><div class="video-meta"><span>1080 × 1920</span><span>30 FPS</span><span>예상 비용 $0</span></div><div class="actions"><a href="/api/video-packages/'+v.id+'/files/hyperframes-input.json" download>문제 발생 시 입력 JSON</a></div></div>'}
...
el('video').onclick=()=>{setStatus('이 PC에서 5초 쇼츠를 렌더링하고 있습니다. 완료되면 MP4가 자동으로 다운로드됩니다.');showToast('5초 쇼츠 렌더링을 시작했습니다.')};
el('date').onchange=updateButton;el('slot').onchange=updateButton;
async function init(){
  el('date').value=new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Seoul'}).format(new Date());renderSources();
  const jobs=[loadSources(),api('/api/prompts').then(p=>{el('prompt').value=p.prompts.unified_post;el('common').textContent=p.common}),api('/api/capabilities').then(c=>{el('cap').innerHTML='<span>API 키: <b>'+esc(c.apiKeyName||'미설정')+'</b></span><span>텍스트 모델: <b>'+esc(c.textModel.id)+' '+(c.textModel.accessible?'설정됨':'키 필요')+'</b></span><span>HyperFrames: <b>로컬 렌더 · $0</b></span><span>저장소: <b>Cloudflare KV</b></span>'})];
