...
type PointLedgerTransaction = Parameters<Parameters<Database["db"]["transaction"]>[0]>[0];

async function applyEntryWithinTransaction(
  transaction: PointLedgerTransaction,
  command: ApplyPointLedgerEntryCommand,
  ...
  const isForcedNegative =
    command.amount < 0 && (command.entryType === "REVERSAL" || command.entryType === "ADJUSTMENT");
  ...
  return {
    async applyEntry(command: ApplyPointLedgerEntryCommand): Promise<PointLedgerMutationResult> {
      return database.transaction((transaction) =>
        applyEntryWithinTransaction(transaction, command),
      );
    },
    applyEntryWithinTransaction,
  };
}
