...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  if (!token && !subjectId) {
    return NextResponse.json(
      { code: "REACTION_SUBJECT_REQUIRED", message: "투표 참여 정보가 필요합니다." },
      { status: 401 },
    );
  }
  ...
  try {
    const upstream = await fetchWhichApi(
      `/v1/comments/${encodeURIComponent(commentId)}/reactions/dislike`,
      {
        method: "POST",
        headers: {
    ...
    return NextResponse.json(
      { code: "API_UNAVAILABLE", message: "싫어요 상태를 바꾸지 못했습니다." },
      { status: 502 },
    );
  }
}
